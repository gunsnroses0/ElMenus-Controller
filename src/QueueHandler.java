...
public class QueueHandler extends ChannelInboundHandlerAdapter {
	...
	public void channelRead(ChannelHandlerContext ctx, Object msg) throws Exception {
		...
		mqChannel.queueDeclare(service + "-controllerResponse", false, false, false, null);
		...
		mqChannel.basicConsume(service + "-controllerResponse", false, consumer);
	}
	...
	private void sendMessage(String service, String requestId, String message) {
		try {
			System.out.println("QUEUEUEUEU"+message);
			mqChannel.queueDeclare(service + "-controller", false, false, false, null);
			AMQP.BasicProperties props = new AMQP.BasicProperties.Builder().correlationId(requestId)
					.replyTo(service + "-controllerResponse").build();
			System.out.println("Sent: " + message);
			mqChannel.basicPublish("", service + "-controller", props, message.getBytes());
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
